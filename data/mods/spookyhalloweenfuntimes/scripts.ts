		...
		runSwitch(pokemon) { // modified for Hoard, Afterimage
			...
			if (pokemon.m.afterimage) {
				if (!pokemon.hasType('Ghost') && pokemon.addType('Ghost')) {
					this.battle.add('-start', pokemon, 'typeadd', 'Ghost', '[from] Ability: Afterimage', '[of] ' + pokemon.m.afterimage);//?
				}
			}
...
/* I'm just gonna block comment the Poultergeist content for now because I dont know what need change and I fear
